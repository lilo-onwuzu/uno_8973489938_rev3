...
class CameraMan {
  ...
  func setupDevices() {
    ...
    .devices()
    .filter {
        return ($0 as AnyObject).hasMediaType(AVMediaType.video)
        }.forEach {_ in 
//        switch ($0 as AnyObject).po {
//      case .front:
//        self.frontCamera = try? AVCaptureDeviceInput(device: $0 as! AVCaptureDevice)
//      case .back:
//        self.backCamera = try? AVCaptureDeviceInput(device: $0 as! AVCaptureDevice)
//      default:
//        break
//      }
    }
...
//      if input.device.supportsSessionPreset(AVCaptureSession.) && self.session.canSetSessionPreset(AVCaptureSession.Preset(rawValue: asset)) {
